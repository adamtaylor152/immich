...
2. In Immich → **Administration → System Settings → Machine Learning → Cloud GPU (RunPod)**:
   ...
   - For **Serverless** mode: provide an ordered list of GPU type IDs (RunPod tries each until one is available). Adjust `Min workers` (default `0` for true scale-to-zero), `Max workers`, and `Idle timeout` if the defaults don't fit.
